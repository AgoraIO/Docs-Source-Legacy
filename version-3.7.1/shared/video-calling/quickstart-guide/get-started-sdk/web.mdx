...
- A valid Agora [account](../reference/manage-agora-account#sign-up-and-log-in?platform=Web).
- An [Agora project](../reference/manage-agora-account#get-started-with-agora) with an App ID and a temp token
- A Windows or macOS computer meets the following requirements:
  ...
  - A browser that matches the [supported browser list](https://docs.agora.io/en/Interactive%20Broadcast/faq/browser_support?platform=Web). Agora highly recommends using the [latest version](https://www.google.com/chrome/) of Google Chrome.
...
1. Download the latest [Agora Web SDK](https://docs.agora.io/en/Video/downloads?platform=Web).
...
- Stream objects, which represent the local and remote [streams](../reference/glossary#media-stream). The Stream methods define the behaviors of a stream object, such as the playback control and video encoder configurations. When you call a Stream method, you need to distinguish between the local and the remote streams.
...
- `mode` determines the [channel profile](../reference/glossary?platform=Web#channel-profile). Agora recommends using the `rtc` mode for one-to-one or group calls and the `live` mode for interactive live streaming.
...
- [How can I set the log file?](https://docs.agora.io/en/Video/faq/logfile)
- [How can I implement call notification in a call application?](https://docs.agora.io/en/Video/faq/call_invite_notification)
- [What are the common errors in Web browsers' console log?](https://docs.agora.io/en/Video/faq/console_error_web)
- [Why do errors occur when calling the Stream.init method?](https://docs.agora.io/en/Video/faq/streaminit_error)
