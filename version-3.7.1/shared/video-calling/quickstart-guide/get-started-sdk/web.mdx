...
- A Windows or macOS computer meets the following requirements:
  ...
  - A browser that matches the [supported browser list](./faq/browser_support?platform=Web). Agora highly recommends using the [latest version](https://www.google.com/chrome/) of Google Chrome.
...
1. Download the latest [Agora Web SDK](./downloads?platform=Web).
...
Once the project is set up, use the core APIs of the Agora Web SDK in `script.js` to implement the basic video call function.

You need to work with two types of objects when using the Agora Web SDK:

- The client object, which represents the local client. The Client methods provide the major functions for a voice/video call, such as joining a [channel](https://docs.agora.io/en/Agora%20Platform/terms#channel) and [publishing](https://docs.agora.io/en/Agora%20Platform/terms#pub) a stream.
- Stream objects, which represent the local and remote [streams](https://docs.agora.io/en/Agora%20Platform/terms#media-stream). The Stream methods define the behaviors of a stream object, such as the playback control and video encoder configurations. When you call a Stream method, you need to distinguish between the local and the remote streams.

The major steps for implementing a basic video call are as follows:

1. Create and initialize a client object.
2. Join a channel.
3. Create a local video stream and publish it.
...
![Web video call API sequence](https://web-cdn.agora.io/docs-files/1592906541341)
...
Create and initialize a client object to control the call. You need to replace `yourAppId` with the App ID of your Agora project. See [Get an App ID](./run_demo_video_call_web?platform=Web#appid) for details.
...
- `mode` determines the [channel profile](https://docs.agora.io/en/Agora%20Platform/terms?platform=Web#channel-profile). Agora recommends using the `rtc` mode for one-to-one or group calls and the `live` mode for interactive live streaming.
...
- For testing, you can [generate a temporary token](./run_demo_video_call_web?platform=Web#temptoken) in Agora Console.
- For production, Agora recommends using a token generated at your server. See [Generate a Token](./token_server?platform=Web).
...
<div class="alert note"><li>Running the web app through a local server (localhost) is for testing purposes only. In production, ensure that you use the HTTPS protocol when deploying your project.</li>
  <li>Due to security limits on HTTP addresses except 127.0.0.1, Agora Web SDK only supports HTTPS or <code>http://localhost</code> (<code>http://127.0.0.1</code>). Do not deploy your project over HTTP.</li></div>
...
4. Allow the browser to access your microphone and camera in the pop-up window. You should see a video stream of yourself.
...
For scenarios involving group video calls, refer to the step-by-step tutorial on how to [build a group video call web app](https://www.agora.io/en/blog/building-a-group-video-chat-web-app/).
...
With the [Agora Video for WordPress](https://wordpress.org/plugins/wp-agora-io/) plugin, you can easily embed live video streaming in your WordPress posts and pages. See the [Agora WordPress Plugin QuickStart Guide](https://www.agora.io/en/blog/agora-video-for-wordpress-plugin-quickstart-guide/).

### FAQ

- [How can I set the log file?](/en/faq/logfile)
- [How can I implement call notification in a call application?](/en/faq/call_invite_notification)
- [What are the common errors in Web browsers' console log?](/en/faq/console_error_web)
- [Why do errors occur when calling the Stream.init method?](/en/faq/streaminit_error)
