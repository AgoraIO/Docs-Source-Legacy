...
This article describes how to create a simple Cocos2d-x project and integrate the Agora Voice SDK to build an iOS app with the basic live audio streaming function.
...
2. Copy the following dynamic libraries from the `audio/libs/ios` folder of the downloaded SDK package to `./sdk/ios/agora` of your project:
...
Agora provides an open-source [Agora-Cocos-Quickstart/Cocos2d-x](https://github.com/AgoraIO-Community/Agora-Cocos-Quickstart/tree/master/Cocos2d-x) sample project on GitHub that implements a basic voice call. You can refer to this sample project and the sample code in this article to implement the interactive live audio streaming.
