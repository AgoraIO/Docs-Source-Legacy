...
- An active Agora project with an App ID and a temporary token. For details, see [Get Started with Agora](https://docs.agora.io/en/Agora Platform/get_appid_token?platform=All Platforms).
- A computer with access to the internet. If your network has a firewall, follow the instructions in [Firewall Requirements](https://docs.agora.io/en/Agora Platform/firewall?platform=All Platforms).
...
Agora provides an open-source sample project [JoinChannelVideo](https://github.com/AgoraIO/API-Examples/blob/master/Android/APIExample/app/src/main/java/io/agora/api/example/examples/basic/JoinChannelVideo.java) on GitHub that implements interactive live video streaming for your reference.
