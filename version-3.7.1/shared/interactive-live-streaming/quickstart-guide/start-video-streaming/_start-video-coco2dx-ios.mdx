<PlatformWrapper platform="cocos2d-x">

This article describes how to create a simple Cocos2d-x project and integrate the Agora Video SDK to build an iOS app with the basic live video streaming function.
...
A mobile device running iOS 9.0 or later. Agora recommends you run this sample project on a physical mobile device, as some simulators may not support the full features of this project.
...
2. Copy the following dynamic libraries from the `video/libs/ios` folder of the downloaded SDK package to `./sdk/ios/agora` of your project:
 ...
 - `AgoraSoundTouch.framework`
  ...
  <div class="alert note">If you need to support a simulator, then copy the above dynamic libraries from the <code>video/libs/ios/ALL_ARCHITECTURE</code> folder to <code>./sdk/ios/agora</code> of your project instead. <br/>The dynamic libraries under this path contain the x86_64 architecture, which affects the distribution of your app in the App Store. Therefore, you need to remove the x86_64 architecture in the libraries before uploading the app to the App Store.</div> 
...
2. Define App ID and token.
   To create and initialize an `IRtcEngine` object, you need to pass in the App ID of your Agora project. See [Get an App ID](https://docs.agora.io/en/Agora%20Platform/get_appid_token?platform=All%20Platforms#get-the-app-id) for details.
	 To ensure communication security, you need to use tokens (dynamic keys) to authenticate users joining a channel.	 
   - For testing, you can [generate a temporary token](https://docs.agora.io/en/Agora%20Platform/get_appid_token?platform=All%20Platforms#generate-an-rtc-temporary-token) in Agora Console.
...
- **The join-channel button**
	...
	```cpp
	// Create a join-channel button. Clicking this button triggers the
	// onJoinChannelClicked() function.
	// You need to add the Button.png and ButtonPressed.png images to the
	// ./Resources folder.
	auto joinButton = cocos2d::ui::Button::create("Button.png", "ButtonPressed.png",
												"ButtonPressed.png");
