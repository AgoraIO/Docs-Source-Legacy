...
<div class="alert info">This article only applies to the Agora Web SDK 3.x and earlier versions. </div>
...
- The client object, which represents the local client. The Client methods provide the major functions for a voice/video call, such as joining a [channel](https://docs.agora.io/en/Agora%20Platform/terms?platform=Web#channel) and [publishing](https://docs.agora.io/en/Agora%20Platform/terms?platform=Web#pub) a stream.
- Stream objects, which represent the local and remote [streams](https://docs.agora.io/en/Agora%20Platform/terms?platform=Web#media-stream). The Stream methods define the behaviors of a stream object, such as the playback control and video encoder configurations. When you call a Stream method, you need to distinguish between the local and the remote streams.
...
Create and initialize a client object to control the call. You need to replace `yourAppId` with the App ID of your Agora project. See [Get an App ID](./run_demo_live_web?platform=Web#appid) for details.
...
- For testing, you can [generate a temporary token](./run_demo_live_web?platform=Web#temptoken) in Agora Console.
...
<div class="alert note">Due to security limits on HTTP addresses except 127.0.0.1, Agora Web SDK only supports HTTPS or <code>http://localhost</code> (<code>http://127.0.0.1</code>). Do not deploy your project over HTTP.</div>
...
4. If the user role is set as host, you need to allow the browser to access your microphone and camera in the pop-up window. You should see a video stream of yourself.
...
- [How can I listen for an audience joining or leaving a live streaming channel?](/en/faq/audience_event)
- [How can a host change the role of a remote user?](/en/faq/host_set_role)
