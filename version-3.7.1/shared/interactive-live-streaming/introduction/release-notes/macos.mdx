...
You can use the `setVideoSource` method to customize a video source, that is, use the MediaIO method for custom video capture. To enhance the flexibility of video capture, as of this release, Agora supports switching from custom video capture by MediaIO to SDK capture directly in the channel. See [How can I switch from custom video capture to SDK capture](./faq/custom_switch_default).
