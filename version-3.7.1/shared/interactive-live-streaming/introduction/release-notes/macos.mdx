...
- Install AgoraALD. See [AgoraALD Virtual Sound Card](./agoraald?platform=macOS).
...
If you are integrating by using the [SDK download link](./downloads?platform=macOS), when you upgrade to v3.6.2, you must modify the project as follows:
...
For integration details, see the [Quickstart Guide](https://docs.agora.io/en/Interactive%20Broadcast/start_live_audio_mac?platform=macOS).
...
- Reporting streaming states, errors, and events:
    ...
    - Adding the following to the `AgoraRtmpStreamingErrorCode` error code:
        ...
        - `AgoraRtmpStreamPublishErrorInvalidAppId(15)`: Your App ID does not have permission to use the Media Push function. Refer to [Prerequisites](https://docs.agora.io/en/Interactive%20Broadcast/cdn_streaming_apple?platform=macOS#prerequisites) to enable the Media Push permission.
...
- [`getDefaultAudioDevice`](./API%20Reference/oc/Classes/AgoraRtcEngineKit.html#//api/name/getDefaultAudioDevice:)

- [`followSystemPlaybackDevice`](./API%20Reference/oc/Classes/AgoraRtcEngineKit.html#//api/name/followSystemPlaybackDevice:)

- [`followSystemRecordingDevice`](./API%20Reference/oc/Classes/AgoraRtcEngineKit.html#//api/name/followSystemRecordingDevice:)
...
<div class="alert note">In this release, a memory leak occurs when `AgoraRtcEngineKit` strongly references `AgoraRtcEngineDelegate`. If you set the strong reference to `AgoraRtcEngineDelegate` by <code>AgoraRtcEngineKit</code>, Agora recommends that you upgrade the SDK to v3.5.0.1. For details, see <a href="#v3.5.0.1">v3.5.0.1</a >.</div>
...
- The values of the `state` parameter in the [`stateChanged`](./API%20Reference/oc/Protocols/AgoraRtcEngineDelegate.html#//api/name/rtcEngine:device:type:stateChanged:) callback
...
- [`adjustLoopbackRecordingSignalVolume`](./API%20Reference/oc/Classes/AgoraRtcEngineKit.html#//api/name/adjustLoopbackRecordingSignalVolume:)
...
- [Error Code](./error_rtc?platform=macOS): `AgoraErrorCodeAlreadyInRecording(160)`
...
This release adds the `setVoiceConversionPreset` method to set a voice conversion effect (to disguise a user's voice). You can use this method to make a male-sounding voice sound steady or deep, and a female-sounding voice sound gender-neutral or sweet. See [Set the Voice Effect](./voice_changer_apple?platform=macOS).
...
-  [Use Cloud Proxy](./cloudproxy_native)
