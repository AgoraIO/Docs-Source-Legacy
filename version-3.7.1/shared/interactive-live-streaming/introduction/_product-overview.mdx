...
Agora Interactive Live Streaming is different from [Agora Video Call](/3.7.1/video-calling/introduction/product-overview). In a video call, all users are the same role and can talk to each other freely. In an interactive live streaming, users can be the host or audience, where only the host can talk. For details, see this [FAQ](https://docs.agora.io/en/faq/profile_difference).
...
| Web                  | See [Web SDK Compatibility](https://docs.agora.io/en/Interactive%20Broadcast/faq/browser_support?platform=Web)) |
