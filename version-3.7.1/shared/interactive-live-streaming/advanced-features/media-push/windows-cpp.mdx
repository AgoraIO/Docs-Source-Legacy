...
1. Refer to [Start Interactive Live Video Streaming](start_live_windows) or [Start Interactive Live Audio Streaming](start_live_audio_windows) to implement the basic audio and video interaction. The `uid` parameter in the `joinChannel` method cannot be empty or 0; otherwise, the hosts in the channel cannot push any media streams to CDN.
