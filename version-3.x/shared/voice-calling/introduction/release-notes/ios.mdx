...
- Occasional crashes.
- A naming conflict existed in the `AgoraReachability` class between the Agora <Vg k="ASDK" /> and Agora <Vg k="MESS_SDK" />.
...
To upgrade the <Vg k="VIDEO" /> Native SDK, you must re-integrate the dynamic library, `AgoraRtcKit.framework`. This process should take no more than five minutes. See [Migration Guide](migration_apple).
