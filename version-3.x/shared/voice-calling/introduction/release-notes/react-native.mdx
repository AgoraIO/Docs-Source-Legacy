...
- [`UnPublishOK (100)`](https://api-ref.agora.io/en/voice-sdk/react-native/3.x/enums/rtmpstreamingerrorcode.html#unuublishok) in `RtmpStreamingErrorCode` 
...
To improve user experience, this version disables the local network connection quality report by default, to prevent the prompt to find local network devices from popping up when an end user launches the app on an iOS 14.0 device. The `gatewayRtt` parameter in the [`RtcStats`](https://api-ref.agora.io/en/video-sdk/react-native/3.x/interfaces/rtcstats.html) callback is disabled by default. Do not use [`gatewayRtt`](https://api-ref.agora.io/en/video-sdk/react-native/3.x/interfaces/rtcstats.html#gatewayrtt) to obtain the round-trip delay between the client and the local router. See the [FAQ](https://docs.agora.io/en/faq/local_network_privacy) for details.
