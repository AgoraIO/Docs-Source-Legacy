...
<div class="alert note">Open the specified ports in <Link to="/3.x/shared/interactive-live-streaming/reference/firewall?platform={{platform.PATH}}">Firewall Requirements</Link> if your network has a firewall.</div>
...
- Go to [SDK Downloads](https://docs.agora.io/en/Voice/downloads?platform=Windows), download the latest version of the Agora SDK for Windows, and unzip the downloaded SDK package.
...
In this step, you need to use the App ID of your project. Refer to <Link to="/3.x/shared/common/interactive-live-streaming/reference/manage-agora-account?platform={{platform.PATH}}">Get Started With Agora</Link> to create an Agora project and get the App ID in Console.
