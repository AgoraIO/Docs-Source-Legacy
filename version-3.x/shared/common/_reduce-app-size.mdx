...
This page describes how to reduce the app size after integrating the Agora <Vg k="VSDK" /> Native SDK.
...
When you integrate the SDK by using the [SDK download link on the official website](https://docs.agora.io/en/sdks?platform=android), you can directly remove the unnecessary extension library files.
...
1. Go to [SDK Downloads](https://docs.agora.io/en/sdks?platform=android), download the latest version of the Agora Voice or Video SDK, and extract the files from the downloaded SDK package.
...
As of v3.7.0, the SDK provides the `startScreenCapture` method and the `AgoraReplayKitExtension.xcframework` extension library for screen sharing. See [Share the Screen](/3.x/video-calling/basic-features/screensharing?platform=ios).
