...
Web only supports customizing video sources.
...
Before customizing the video source and renderer, ensure that you have implemented the basic real-time communication functions. For details, see [Start a Video Call](/3.x/video-calling/quickstart-guide/get-started-sdk) or [Start Interactive Video Streaming](../quickstart-guide/start-video-streaming).

### Customize the audio/video source

When creating a stream with the `createStream` method, you can specify customized audio/video sources by the [`audioSource`](https://api-ref.agora.io/en/video-sdk/web/3.x/interfaces/agorartc.streamspec.html#audiosource) and [`videoSource`](https://api-ref.agora.io/en/video-sdk/web/3.x/interfaces/agorartc.streamspec.html#videosource) properties.
...
Call the [`Stream.getVideoTrack`](https://api-ref.agora.io/en/video-sdk/web/3.x/interfaces/agorartc.stream.html#getvideotrack) method and attach the track to the local canvas.
