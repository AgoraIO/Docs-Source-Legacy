...
Agora provides an open-source sample project on GitHub that implements the <Link to="{{GLOBAL.AGORA_API_EXAMPLES_WINDOWS_TREE}}/Advanced/ReportInCall">in-call statistics monitoring</Link> function. You can download the sample project to try it out or view the source code.
