...
Agora provides an open-source sample project ([VoiceEffects.java](https://github.com/AgoraIO/API-Examples/tree/legacy/Android/APIExample/app/src/main/java/io/agora/api/example/examples/advanced/VoiceEffects.java)) that implements the voice beautifier and audio effects functions on GitHub. You can download it to try out this function and view the source code.
...
- [`setVoiceConversionPreset`](https://api-ref.agora.io/en/video-sdk/macos/3.x/Classes/AgoraRtcEngineKit.html?platform=iOS#//api/name/setVoiceConversionPreset:)
