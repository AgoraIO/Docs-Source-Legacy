...
Agora provides an open-source sample project that implements <Link to="{{GLOBAL.AGORA_API_EXAMPLES_WINDOWS_TREE}}/Advanced/MediaEncrypt">built-in encryption</Link> and <Link to="{{GLOBAL.AGORA_API_EXAMPLES_WINDOWS_TREE}}/Advanced/CustomEncrypt">customized encryption</Link> on GitHub. You can try the demo and view the source code.
