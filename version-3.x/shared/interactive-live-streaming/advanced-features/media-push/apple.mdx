...
4. Call `stopRtmpStream` and pass in the CDN address to stop the current live streaming.
   ...
   <li>After stopping the streaming, you can receive the <code>rtmpStreamingChangedToState</code> callback reporting the streaming state as <code>IDLE(0)</code>. If you receive another streaming state, see <a href="../best-practices/exception-handling-for-media-push">Exception Handling for Media Push</a>.</li>
   </ul></div>
...
- [`streamPublishedWithUrl`](https://docs.agora.io/en/Interactive%20Broadcast/API%20Reference/oc/Protocols/AgoraRtcEngineDelegate.html#//api/name/rtcEngine:streamPublishedWithUrl:errorCode:)
- [`streamUnpublishedWithUrl`](https://docs.agora.io/en/Interactive%20Broadcast/API%20Reference/oc/Protocols/AgoraRtcEngineDelegate.html#//api/name/rtcEngine:streamUnpublishedWithUrl:)
