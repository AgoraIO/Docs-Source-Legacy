...
Agora provides an [open-source demo project](https://github.com/AgoraIO/API-Examples/blob/main/Android/APIExample/app/src/main/java/io/agora/api/example/examples/advanced/PlayAudioFiles.java) on GitHub that implements playing audio effect files and a music file. You can view the source code on Github or download the project to try it out.
