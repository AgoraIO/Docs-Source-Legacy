...
Refer to [the RTC quickstart guide](/en/3.x/interactive-live-streaming/quickstart-guide/start-video-streaming#4-set-the-channel-profile) for details about how to enable the local user to join the `LIVE_BROADCASTING` channel in the role of `BROADCASTER`:
