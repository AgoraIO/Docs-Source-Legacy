...
- Xcode 9.0 or later
- A macOS device running macOS 10.12 or later
...
1. Go to [Downloads](https://docs.agora.io/en/sdks?platform=macos), download the latest version of the MediaPlayer Kit, and extract the files from the download package.
...
<div class="alert note">If you use V1.2.4 or later, you need to add <b>AgoraPlayerFFmpeg.framework</b>.</div>
...
Integration steps: See [Integrate the Native SDK](/en/3.x/interactive-live-streaming/quickstart-guide/start-video-streaming#a-nameintegratesdkaintegrate-the-sdk).
...
1. [Instantiate an AgoraRtcEngineKit object](/en/3.x/interactive-live-streaming/quickstart-guide/start-video-streaming#3-initialize-agorartcenginekit).
...
Refer to [the <Vg k="VSDK" /> quickstart guide](/en/3.x/interactive-live-streaming/quickstart-guide/start-video-streaming#4-set-the-channel-profile) for details about how to enable the local user to join the `LIVE_BROADCASTING` channel in the role of `BROADCASTER`:
...
## API documentation
See the [API documentation](https://api-ref.agora.io/en/mediaplayer-kit/macos/1.x/docs/headers/MediaPlayer-Kit-Objective-C-API-Overview.html).
