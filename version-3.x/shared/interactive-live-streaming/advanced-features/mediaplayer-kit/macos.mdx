...
1. Go to [Downloads](https://docs.agora.io/en/Interactive%20Broadcast/downloads?platform=macOS), download the latest version of the MediaPlayer Kit, and extract the files from the download package.
...
Integration steps: See [Integrate the Native SDK](../quickstart-guide/start-video-streaming#a-nameintegratesdkaintegrate-the-sdk).
...
1. [Instantiate an AgoraRtcEngineKit object](../quickstart-guide/start-video-streaming#3-initialize-agorartcenginekit).
...
Refer to [the RTC quickstart guide](../quickstart-guide/start-video-streaming#4-set-the-channel-profile) for details about how to enable the local user to join the `LIVE_BROADCASTING` channel in the role of `BROADCASTER`:
