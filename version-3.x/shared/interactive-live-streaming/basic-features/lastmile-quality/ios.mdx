...
As of v2.4.0, the Agora <Vg k="VIDEO" /> Native SDK provides the `startEchoTestWithInterval` method that tests whether network connections and audio devices, such as the microphone and speaker, are working properly.
...
As of v3.5.2, the Agora <Vg k="VIDEO" /> Native SDK provides the `startEchoTestWithConfig` method that tests whether the system's audio devices, video devices, and network connections are working properly.
