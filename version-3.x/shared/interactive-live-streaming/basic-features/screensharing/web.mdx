...
<div class="alert note">The Agora <Vg k="VIDEO" /> Web SDK supports screen sharing in the following <b>desktop</b> browsers:<li>Chrome 58 or later</li><li>Firefox 56 or later</li><li>Edge 80 or later on Windows 10+</li></div>
...
Before you begin, ensure that you understand how to [start a video call](/3.x/video-calling/quickstart-guide/get-started-sdk) or [start interactive video streaming](/en/3.x/interactive-live-streaming/quickstart-guide/start-video-streaming).
...
1. Add the [Google Chrome Extension for Screen Sharing](../basic-features/screensharing#screen-sharing-on-chrome) provided by Agora and get an extension ID.
...
2. Call `localStream.init` to initialize the stream.
   ...
   ![1571629977600](https://web-cdn.agora.io/docs-files/1571629977600)
...
<div class="alert note">Note:<ul>
	<li>The <code>audio</code> and <code>screenAudio</code> properties are different: <ul><li><code>audio</code> controls whether the stream contains the audio captured by the local audio input device.</li><li><code>screenAudio</code> controls whether the stream contains the local audio playback.</li></ul>Agora recommends setting <code>audio</code> as <code>false</code> in the screen-sharing stream. If you set both <code>audio</code> and <code>screenAudio</code>  as <code>true</code>, the stream contains the local audio playback only.</li>
	...
	<li>Audio sharing is disabled when the user shares a single application window.</li></ul></div>

![1574232834184](https://web-cdn.agora.io/docs-files/1574232834184)
...
<img alt="../_images/screensharing_streams.png" src="https://web-cdn.agora.io/docs-files/en/screensharing_streams.png"/>
...
- [How can I switch between the screen-sharing stream and the camera stream?](https://docs.agora.io/en/help/integration-issues/switch_screen_camera_web)
