...
Apps need to provide different audio experiences according to different business needs. In order to provide these different audio experiences, you can use the Agora <Vg k="VSDK" /> to set the appropriate audio profile ([`profile`](https://api-ref.agora.io/en/video-sdk/cpp/3.x//namespaceagora_1_1rtc.html#a75448342947f6e98becc2b27d2edca55)) and audio scenario ([`scenario`](https://api-ref.agora.io/en/video-sdk/cpp/3.x//namespaceagora_1_1rtc.html#a2c5708461a9e1568bbda636d95055533)).
...
Before setting the audio profile and scenario, ensure that you have implemented the basic real-time communication functions in your project. For details, see [Start a Call](/3.x/video-calling/quickstart-guide/get-started-sdk) or [Start Live Interactive Streaming](/en/3.x/interactive-live-streaming/quickstart-guide/start-video-streaming).
...
```cpp
rtcEngine.setAudioProfile(AUDIO_PROFILE_DEFAULT, AUDIO_SCENARIO_DEFAULT);
```
...
```cpp
rtcEngine.setAudioProfile(AUDIO_PROFILE_SPEECH_STANDARD, AUDIO_SCENARIO_CHATROOM_GAMING);
```
...
```cpp
rtcEngine.setAudioProfile(AUDIO_PROFILE_MUSIC_STANDARD, AUDIO_SCENARIO_CHATROOM_ENTERTAINMENT);
```
...
```cpp
rtcEngine.setAudioProfile(AUDIO_PROFILE_MUSIC_HIGH_QUALITY, AUDIO_SCENARIO_GAME_STREAMING);
```
...
```cpp
rtcEngine.setAudioProfile(AUDIO_PROFILE_MUSIC_HIGH_QUALITY_STEREO, AUDIO_SCENARIO_SHOWROOM);
```
...
```cpp
rtcEngine.setAudioProfile(AUDIO_PROFILE_MUSIC_STANDARD_STEREO, AUDIO_SCENARIO_GAME_STREAMING);
```
...
```cpp
rtcEngine.setAudioProfile(AUDIO_PROFILE_MUSIC_STANDARD_STEREO, AUDIO_SCENARIO_CHATROOM_ENTERTAINMENT);
```
...
Agora provides an open-source demo project [AudioProfile](https://github.com/AgoraIO/API-Examples/tree/master/windows/APIExample/APIExample/Advanced/AudioProfile) that implements setting the audio profile and scenario on GitHub. You can download the sample project and view the source code.

#### API reference

- [`setAudioProfile`](https://api-ref.agora.io/en/video-sdk/cpp/3.x//classagora_1_1rtc_1_1_i_rtc_engine.html#ab0cb52e238b729a15525a5cc12543d9e)
