import NotSupported from '@docs/shared/common/no-known-issues.mdx';
...
2. When calling `createMicrophoneAudioTrack`, set `bypassWebAudio` as `true`. The Web SDK directly publishes the local audio stream without processing it through `WebAudio`.
   ...
   > Moreover, this workaround applies for Safari in iOS only. The reason is that Chrome on iOS uses WebView but Safari uses Webkit. WebView is an embeddable browser that a native application can use to display web content. Webkit is more performant, has more features, more privileges than WebView.
</details>
