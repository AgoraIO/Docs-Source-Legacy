...
- Calling `createScreenVideoTrack` to enable screen sharing has the following restrictions:
   - The Web SDK supports screen sharing on Chrome 58 or later. If the Chrome version is earlier than 72, you need to add the [Google Chrome Extension for Screen Sharing](/en/Interactive%20Broadcast/chrome_screensharing_plugin) provided by Agora, get the `extensionId`, and set the `extensionId` parameter when you create a video track for screen sharing. If the Chrome version is 72 or later, simply call `createScreenVideoTrack`.
