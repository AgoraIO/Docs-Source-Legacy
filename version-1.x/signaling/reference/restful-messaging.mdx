...
You need to pass the basic HTTP authentication and put `api_key:api_secret` in the `Authorization` field of the HTTP header. For more information on how to generate the <code>Authorization</code> field, see [RESTful authentication](/1.x/signaling/reference/restful-authentication).
...
For more information about how to generate a Signaling Token, see [Secure authentication with tokens](/1.x/signaling/develop/authentication-workflow).
...
- `Authorization` is basic HTTP or token. See [RESTful authentication](/1.x/signaling/reference/restful-authentication).
