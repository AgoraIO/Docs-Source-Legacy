...
You need to pass the basic HTTP authentication and put `api_key:api_secret` in the `Authorization` field of the HTTP header. For more information on how to generate the <code>Authorization</code> field, see [Secure authentication with tokens](/1.x/signaling/reference/restful-authentication).
